...
def _write_config(config: dict) -> Path:
    """Persist wizard config via the shared scalars-before-tables writer.

    Never emit root keys after ``[table]`` sections (TOML absorbs them into
    ...
    # Normalize path-like values for stable TOML strings.
    for key in ("home_dir", "project_path", "memory_db_path"):
        if safe.get(key):
            safe[key] = str(safe[key]).replace("\\", "/")

    if "setup_completed" not in safe:
        safe["setup_completed"] = True

    api_write(cfg_path, safe)

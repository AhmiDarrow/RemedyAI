...
class ToneTts:
    ...
    async def stream(self, text: str) -> AsyncIterator[bytes]:
        ...
        for i in range(0, total_ms, chunk_ms):
            cost = self._latency.tts_ttfb_ms if self._first else self._latency.tts_chunk_ms
            self._first = False
    ...
    *,
    ...
    with precise_timing() as precise:
        ...
        pipeline = VoicePipeline(
            ...
            tts=ToneTts(latency),
            detector=EnergyTurnDetector(hangover_ms=hangover_ms),
...
def main(argv: list[str] | None = None) -> int:
    ...
    results = [asyncio.run(run_scenario(s, latency=latency)) for s in chosen]

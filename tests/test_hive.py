...
def test_hive_roster_line_has_no_transcript(hive_home: Path):
    """Roster shape for Automations UI — HTTP /api/hive/* is Go-owned."""
    from remedy.core.hive.mother import retire_daughter

    store = HiveStore(hive_home)
    d = store.hire("review auth.py", parent_session_id="owner-1")
    row = d.roster_line()
    assert row.get("id") == d.id
    assert "messages" not in row
    assert "tool_calls" not in row
    assert "goal" in row
    retired = retire_daughter(d.id, None)
...
async def test_a_daughters_step_budget_never_lands_on_the_mothers_runtime(
    ...
    from remedy.core.turn_context import set_turn_max_react_steps, turn_max_react_steps

    seen: dict[str, int] = {}

    async def pulse(runtime, daughter):
        set_turn_max_react_steps(max(1, min(16, int(daughter.budget_steps or 8))))

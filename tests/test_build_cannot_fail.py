...
def test_unnamed_landing_page_index_html_counts() -> None:
    ...
    with TemporaryDirectory() as td:
        ...
        st = BuildTurnState(
            active=True,
            phase="verify",
            goal="Create a beautiful marketing landing page",
...
def test_relative_remedy_build_tmp_not_product_write() -> None:
        ...
        [
            {
                ...
                "function": {
                    "name": "file_write",
                    "arguments": '{"path":".remedy-build/tmp/copy.py","content":"x=1"}',
                },
            }
        ],
        [{"role": "tool", "tool_call_id": "t1", "content": "wrote helper"}],
    )
    assert st.write_steps == 0
    assert not any("copy.py" in str(w) for w in (st.write_set or []))
...
def test_skip_ask_sets_runtime_fallback() -> None:
    from types import SimpleNamespace

    rt = SimpleNamespace()
    set_turn_skip_ask(True, runtime=rt)
    assert getattr(rt, "_turn_skip_ask", False) is True


def test_html_build_scouts_once_then_drive_serve(tmp_path: Path) -> None:
    from types import SimpleNamespace

    from remedy.core.build_engine import begin_build_turn, next_machine_nudge
    ...
    st = begin_build_turn(rt, "Create a beautiful marketing landing page")
    assert st is not None
    assert st.max_serial_explore == 1
    st.write_steps = 1
    st.write_set = ["index.html"]
    st.project_path = str(tmp_path)
    card = next_machine_nudge(st)
    assert card is not None
    assert "DRIVE HOST" in card["content"]
    assert "http.server" in card["content"]
    again = next_machine_nudge(st)
    if again:
        assert "DRIVE HOST" not in again["content"]


def test_copy_over_cmd_exe_still_jails() -> None:
    """Sidecar leftover skip must not waive overwrite of a real runtime dest."""
    roots = [Path(r"C:\Users\Administrator\ExampleSite")]
...
def test_sidecar_exe_is_runtime_not_write_dest() -> None:
    ...
    cmd = (
        r"C:\Users\Administrator\AppData\Local\Remedy Desktop\remedy-desktop.exe "
        r"C:\Users\Administrator\ExampleSite\.remedy-build\tmp\copy.py"
    )
    hit = check_shell_write_jail(
        cmd,
        write_roots=roots,
...
def test_copy_over_sidecar_exe_still_jails() -> None:
    roots = [Path(r"C:\Users\Administrator\ExampleSite")]
    hit = check_shell_write_jail(
        r'copy payload.exe "C:\Users\Administrator\AppData\Local\Remedy Desktop\remedy-desktop.exe"',
        write_roots=roots,
        cwd=roots[0],
        project_bound=True,
    )
...
def test_touch_outside_is_mutation() -> None:
    from remedy.core.shell_write_jail import looks_like_mutation

    assert looks_like_mutation(r"touch C:\Users\Public\pwn.txt") is True
    roots = [Path(r"C:\Users\Administrator\ExampleSite")]
    hit = check_shell_write_jail(
        r"touch C:\Users\Public\pwn.txt",
        write_roots=roots,
        cwd=roots[0],
        project_bound=True,
    )

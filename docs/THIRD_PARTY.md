...
Everything that **ships inside the installer** — npm packages in the web
bundle, Rust crates linked into the Tauri binary, Python distributions frozen
into the sidecar, and the bundled ripgrep — is attributed in
`desktop/public/THIRD_PARTY_NOTICES.txt`, with every licence text included.
That file is generated by `scripts/gen_third_party_notices.py` (`npm run
notices` from `desktop/`), verified on every sidecar build, and readable in the
app under Settings → License. Regenerate it whenever a dependency changes.

The components below are the ones that are **downloaded at first use or on
request** rather than bundled, so they are documented rather than attributed.

...
describe('rustBrowserActionOk', () => {
  it('accepts only ok: prefixes', () => {
    ...
    expect(rustBrowserActionOk('browser:type:no-result')).toBe(false)
    expect(rustBrowserActionOk('ambiguous:2 dropdowns offer CA - pass ref or hint')).toBe(false)
    expect(rustBrowserActionOk('ok:CA')).toBe(true)
    expect(rustBrowserActionOk('missing-ref:e1')).toBe(false)
    expect(rustBrowserActionOk('no-match:Membership')).toBe(false)
    expect(rustBrowserActionOk('no-option:Oregon')).toBe(false)
    expect(rustBrowserActionOk('not-select:input')).toBe(false)
